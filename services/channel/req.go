...
type (
	...
	AddReq struct {
		...
		ReqUrl                     string `validate:"minlength(1,请求url不能为空) maxlength(500,请求url长度不能超过500)" json:"req_url"`                              // 请求url
		ReqChannel                 string `validate:"enum(GET|POST,请求方式不合法)" json:"req_method"`                                                         // 请求方式
		ReqContentType             string `validate:"enum(json|form|urlencoded,请求数据类型不合法)" json:"req_content_type"`                                     // 请求数据类型
		ReqSuccessExpr             string `validate:"minlength(1,请求成功计算表达式不能为空) maxlength(500,请求成功计算表达式长度不能超过500)" json:"req_success_expr"`             // 请求成功计算表达式
		...
		ReqPayMessageExpr          string `validate:"minlength(1,请求支付获取消息表达式不能为空) maxlength(500,请求支付获取消息表达式长度不能超过500)" json:"req_pay_message_expr"`     // 请求支付获取消息表达式
		NotifyPayContentType       string `validate:"enum(json|form|urlencoded,回调支付数据类型不合法)" json:"notify_pay_content_type"`                            // 回调支付数据类型
		NotifyPaySuccessExpr       string `validate:"minlength(1,回调支付成功计算表达式不能为空) maxlength(500,回调支付成功计算表达式长度不能超过500)"  json:"notify_pay_success_expr"` // 回调支付成功计算表达式
